...
def monomial_value(m): # m is a monomial in the ambient Chow ring; output the integral value of this monomial.
...
def keel(m):  # m is a monomial in the ambient Chow ring; output 0 if m fulfills Keel's quadratic relation, output 1 otherwise (i.e. when m is a tree monomial).
...
def keel_micro(m1,m2): #m1,m2 are two tuples representing two elements in the ambient Chow group of codimension one, respectively.
...
def sign(m):# m is a monomial in the ambient Chow ring; output the sign of the integral value of this monomial.
    a = 0
    e = Counter(m)
    l = len(e)
    for x in e:
...
def redundancy_tree(m):# m is a monomial in the ambient Chow ring; output the redundancy tree of this monomial.
    G = nx.DiGraph()
    G1 = nx.Graph()
    P = []
    C = []
...
def absolute_value(g):#g is a redundancy tree    
    g1 = g.nodes   
...
print monomial_value(m2)# Here is one example of how to apply the function "monomial_value" to the monomial m2 given above.
